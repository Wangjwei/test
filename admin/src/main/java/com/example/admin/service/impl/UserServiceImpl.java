...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public void insertUser() throws Exception {
        ...
        userList.forEach(user -> userESService.insertOrUpdateOne(indexName,user));
    }
    ...
    public List<String> searchCompletionSuggest(String searchValue) {
        ...
        result = userESService.searchCompletionSuggest(indexName, getSearchSourceBuilder(field, searchValue, false));
        if (result.size() == 0  && checkLetter(searchValue)) {
            //拼音fuzzy查询
            result = userESService.searchCompletionSuggest(indexName, getSearchSourceBuilder(field, searchValue, true));
            if (result.size() == 0) {
                //首字母查询
                result = userESService.searchCompletionSuggest(indexName,
