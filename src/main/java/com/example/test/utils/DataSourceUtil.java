...
public class DataSourceUtil {
	...
	private static Map<String, DruidDataSource> dataSources = new HashMap<String, DruidDataSource>();
...
//			Connection connection = buildDataSource(dataSource.getDriverclassname(), dataSource.getUrl(), dataSource.getUsername(), dataSource.getPassword(), dataSource.getInitialsize(), null, dataSource.getMaxactive(), dataSource.getMaxwait(), dataSource.getValidationquery()).getConnection();
	...
	public static Connection getConnection(Object ds) throws Exception
	{
		...
		}else if(ds != null && ds instanceof DruidDataSource){
			...
			try {
				for(Object o:objects){
					if(o != null && o instanceof ResultSet){
						((ResultSet)o).close();
					}else if(o != null && o instanceof Statement){
						((Statement)o).close();
					}else if(o != null && o instanceof Connection){
						((Connection)o).close();
					}else if(o != null && o instanceof PreparedStatement){
	...
	public static String pageSql(String sql)
	{
		...
		pageSql.append("select * from (select * from (").append(sql).append(") where rn_ <= ?) where rn_ > ?");
	...
	{
		StringBuilder pageSql = new StringBuilder();
		pageSql.append("select count(1) cc from (").append(sql).append(")");
		return pageSql.toString();
	}
